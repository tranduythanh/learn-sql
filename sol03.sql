...
-- A.a 
IF OBJECT_ID('TongSoLuongNhap') IS NOT NULL DROP PROCEDURE TongSoLuongNhap;
CREATE PROCEDURE TongSoLuongNhap
	@MAHH char(5),
	@From datetime,
	@To   datetime
AS
BEGIN
	SELECT SUM(SOLUONG) FROM CT_HoaDon 
	WHERE 	MAHH = @MAHH AND 
		LEFT(SOHD, 1) = 'N' AND
		SOHD IN (SELECT SOHD FROM HoaDon WHERE NGAYLAPHD >= @From AND NGAYLAPHD <= @To)
END;
EXEC TongSoLuongNhap 'CPU01', '2006/01/01', '2006/07/01';

-- A.b 
IF OBJECT_ID('TongSoLuongXuat') IS NOT NULL DROP PROCEDURE TongSoLuongXuat;
CREATE PROCEDURE TongSoLuongXuat
	@MAHH char(5),
	@From datetime,
	@To   datetime
AS
BEGIN
	...
	WHERE 	MAHH = @MAHH AND 
		...
		SOHD IN (SELECT SOHD FROM HoaDon WHERE NGAYLAPHD >= @From AND NGAYLAPHD <= @To)
END;
EXEC TongSoLuongXuat 'CPU01', '2006/01/01', '2006/07/01';

-- A.c 
IF OBJECT_ID('TongDoanhThuThang') IS NOT NULL DROP PROCEDURE TongDoanhThuThang;
CREATE PROCEDURE TongDoanhThuThang
	@Thang int,
	@Nam   int
AS
BEGIN
SELECT SUM(DONGIA*SOLUONG) FROM CT_HoaDon 
WHERE 	LEFT(SOHD, 1)='X' AND 
		SOHD IN (SELECT SOHD FROM HoaDon WHERE MONTH(NGAYLAPHD)=@Thang AND YEAR(NGAYLAPHD)=@Nam)
END;
EXEC TongDoanhThuThang 5, 2006;

-- A.d 
IF OBJECT_ID('TongDoanhThuHangHoa') IS NOT NULL DROP PROCEDURE TongDoanhThuHangHoa;
CREATE PROCEDURE TongDoanhThuHangHoa 
	@MAHH char(5),
	@From datetime,
	@To   datetime
AS
...
WHERE 	LEFT(SOHD, 1)='X' AND 
		...
		SOHD IN (SELECT SOHD FROM HoaDon WHERE NGAYLAPHD >= @From AND NGAYLAPHD <= @To)
END;
EXEC TongDoanhThuHangHoa 'CPU01', '2006/01/01', '2006/07/01';

-- A.e 
IF OBJECT_ID('TongTienNhapHang') IS NOT NULL DROP PROCEDURE TongTienNhapHang;
CREATE PROCEDURE TongTienNhapHang 
	@From datetime,
	@To   datetime
AS
BEGIN
...
WHERE 	LEFT(SOHD, 1)='X' AND 
		SOHD IN (SELECT SOHD FROM HoaDon WHERE NGAYLAPHD >= @From AND NGAYLAPHD <= @To)
END;
EXEC TongTienNhapHang '2006/01/01', '2006/07/01';

-- A.f
IF OBJECT_ID('TongTienHoaDon') IS NOT NULL DROP PROCEDURE TongTienHoaDon;
